...
// installed state. The ssa subpackage is the implementation; it applies a
// build-time-rendered copy of the Helm chart over the cluster's API by
// server-side apply, so the CLI needs neither the Helm SDK nor a network chart
// pull at runtime.
//
...
// machinery — the embedded manifest filesystem and the server-side-apply client
// — lives entirely in the ssa adapter so consumers depend only on the port.
//
