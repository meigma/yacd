...
// GenesisKind identifies which Cardano genesis hash command should be used for
// a genesis file referenced by configuration.yaml.
...
// GenesisHasher computes a Cardano genesis hash for a local genesis file.
// Implementations may call cardano-cli or provide test doubles.
type GenesisHasher interface {
	HashGenesis(ctx context.Context, kind GenesisKind, genesisPath string) (string, error)
}

// CardanoCLIHasher shells out to cardano-cli for genesis hash computation.
// YACD's tools image already includes cardano-cli, which keeps
// protocol-specific hashing semantics in the upstream tool instead of
...
// EnrichGenesisHashes returns a copy of artifactData whose configuration.yaml
// includes missing genesis hash fields for any referenced genesis files.
func EnrichGenesisHashes(ctx context.Context, envDir string, artifactData map[string]string, hasher GenesisHasher) (map[string]string, error) {
	...
	content, ok := out[networkartifacts.ConfigurationKey]
	if !ok {
		return out, nil
	}
	...
	out[networkartifacts.ConfigurationKey] = configuration
	return out, nil
}
...
//nolint:gochecknoglobals // immutable declarative mapping of config keys.
...
// enrichNodeConfiguration parses content as YAML, computes any missing genesis
// hash fields via hasher, and returns the re-serialized configuration. It
// returns content unchanged when every referenced hash is already present.
...
// artifactPathUnderEnv joins filePath onto envDir, refusing absolute paths or
// paths that escape envDir.
