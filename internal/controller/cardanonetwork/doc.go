...
// kupo chain API sidecars, an always-on cardano-tools serve sidecar
// (and its owned artifacts Service) that exposes the staged network artifacts
// over HTTP, the genesis-funded faucet wallet Secret on local networks, and the
// selected CardanoDBSync primary-sidecar attachment; it then publishes
// endpoints and readiness state through CardanoNetwork status.
//
...
//   - controller.go, apply.go, callbacks.go, delete.go, wallet.go,
//     status.go, readiness.go: side-effecting reconciler. Reads from and
//     writes to the cluster, generates the faucet wallet key material
//     (wallet.go is the only crypto/rand caller), and publishes status.
//
...
// shape with per-component {Node,Ogmios,Kupo,Artifacts}Ready
