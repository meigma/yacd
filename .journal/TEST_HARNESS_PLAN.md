...
|---|---|---|
| 0 — Validate feasibility ✅ **GO** | Evidence the CI path and teardown actually work | Go/no-go for the CI story |
...
> **DONE — GO (session 036, 2026-05-29).** Evidence and the go/no-go decision
> are in [`TEST_HARNESS_PHASE0_RESULTS.md`](./TEST_HARNESS_PHASE0_RESULTS.md).
> Headline: cold-start to Ready **27s** (full pipeline 112s) vs the 10–12m
> budget; teardown GC clean (11 children → 0 in 3s, no finalizers);
> `run`/`exec` host-access empirically confirmed (Ogmios port-forward and in-pod
> `cardano-cli` agree on the same tip). Caveats: measured on a 4 vCPU/16 GB
> `ubuntu-latest` (the 2-core private tier is untested); preload Ogmios/Kupo to
> avoid Docker Hub rate-limit jitter; the documented `test-e2e.sh` path is
> broken (`docker build` + `.dockerignore` strips `go:embed` profiles — use ko).
