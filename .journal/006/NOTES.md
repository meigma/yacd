...
## 2026-05-21 17:07 — Installed-operator CardanoNetwork smoke implemented
Goal for the checkpoint: finish phase-2 remaining-work item 4 by proving the packaged operator can reconcile a representative `CardanoNetwork` in Kind.
Current state of the world: branch `feat/cardanonetwork-primary-service` now has commit `5f21f26` (`test(cardanonetwork): smoke primary workload in kind`) on top of the Service, endpoint-status, and readiness commits. The existing Chainsaw manager smoke now applies `CardanoNetwork/phase2-smoke` in a restricted `yacd-smoke` namespace, asserts the owned PVC and node-to-node Service, waits for `Deployment/phase2-smoke-node` and the selected pod to become ready, and checks `status.endpoints.nodeToNode`, `NodeReady=True`, `Progressing=False`, and `Degraded=False`. Catch diagnostics now include the smoke CR, child resources, events, descriptions, and pod logs. This is a Kubernetes workload readiness proof only; protocol-level `cardano-cli` or socket health remains out of scope for this phase-2 slice.
Validation: ran `chainsaw lint test --file test/chainsaw/manager-smoke/chainsaw-test.yaml`, `moon run root:check`, `moon run root:test`, `moon run root:test-e2e`, and `git diff --check`; all passed. A first `root:test-e2e` run also passed but the timing wrapper exited nonzero after success because it used zsh's read-only `status` variable; reran with a safe wrapper and the clean run passed in 85 seconds. The Kind/Tilt dev stack remains running for the active session.
Plan: remaining phase-2 work is current-state docs drift, especially README text that still says the first reconciler will land later.

## 2026-05-21 17:24 — Close
Goal for the checkpoint: close session 006 after review and merge.
Current state of the world: PR #11 (`feat(cardanonetwork): publish primary node readiness`) was reviewed by the human, CI and Kusari were green, and it was squash-merged to `master` as `c415f3e`. The local `master` checkout was fast-forwarded to the merge commit, the remote feature branch was deleted, and the `feat/cardanonetwork-primary-service` Worktrunk worktree was removed. The session dev stack was stopped with `moon run root:dev-down` before merge cleanup. `SUMMARY.md`, `INDEX.md`, and `TECH_NOTES.md` now record the completed primary node Service/status/readiness/installed-smoke slice and the remaining README docs drift.
Plan: session 006 is complete. Start the next session from updated `master`; the next narrow item is README/current-state docs cleanup unless priorities change.
