...
## 2026-05-27 14:54 — Placement handoff fix implemented and regressed
Implemented the approved fix on Worktrunk branch `test/primary-sidecar-functional`: accepted db-sync placement is now tracked separately from the frozen database identity fingerprint through `status.database.acceptedPlacementMode` and the `yacd.meigma.io/dbsync-placement-mode` owned annotation. The controller rejects post-acceptance `primarySidecar` <-> `dedicatedFollower` changes with `UnsupportedDatabaseIdentityChange`, preserves the no-duplicate workload handoff guards, and backfills legacy accepted placement metadata from existing state/workload shape.

Validation passed: `moon run root:generate`, `moon run root:test`, `moon run root:check`, and `git diff --check`. Committed implementation as `ce73f4b` (`fix(cardanodbsync): reject accepted placement handoffs`).

Manual Kind/Tilt regression passed against the running dev stack. Revalidated managed Postgres primary sidecar and external Postgres primary sidecar, including real Postgres `block` rows. Rechecked placement conflict and conflict recovery. The original sidecar-to-dedicated transition now degrades as `UnsupportedDatabaseIdentityChange`, preserves accepted placement as `primarySidecar`, creates no dedicated `*-dbsync` Deployment, and leaves no db-sync sidecar running after CardanoNetwork reconciles. The reverse accepted dedicated-to-sidecar boundary also degrades as `UnsupportedDatabaseIdentityChange`, preserves accepted placement as `dedicatedFollower`, scales the dedicated Deployment to zero, and does not attach the primary sidecar. Evidence captured under `/tmp/yacd-sidecar-functional-20260527-fix`. Deleted namespace `yacd-sidecar`; left the dev stack running.

## 2026-05-27 15:22 — Close
Closed the session after PR #46 (`fix(cardanodbsync): reject accepted placement handoffs`) was approved, squash-merged as `5b3185a`, and `master` was fast-forwarded locally. Deleted the remote `test/primary-sidecar-functional` branch and removed its Worktrunk worktree. Ran `moon run root:dev-down` before merge cleanup; the Kind/Tilt dev stack is down. Wrote `.journal/026/SUMMARY.md`, updated `.journal/INDEX.md`, and updated `.journal/TECH_NOTES.md` with the accepted-placement contract.
