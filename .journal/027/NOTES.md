...
## 2026-05-27 20:22 — Public primarySidecar clarification
Clarification: `CardanoDBSync.spec.placement.mode: primarySidecar` already exists and was tested for local `CardanoNetwork` resources in sessions 025-026. The phrase "until public primarySidecar exists" should be read as "until primarySidecar is deliberately enabled and validated for public CardanoNetwork profiles." The current public db-sync gate only allows `dedicatedFollower`, and the primary-sidecar validation still requires `CardanoNetwork.status.network.mode=local`. For preview/preprod, public primarySidecar may be a small follow-up slice to lift that gate and verify public artifacts/socket behavior. For mainnet, it is likely a useful way to avoid a second follower bootstrap, but still needs explicit combined resource/storage sizing and manual proof before being treated as supported.

## 2026-05-27 20:44 — Close
Outcome: PR #47 (`feat(cardanonetwork): support public network profiles`, https://github.com/meigma/yacd/pull/47) was reviewed, CI/Kusari passed, and it was squash-merged as `385718b`. Local `master` was fast-forwarded to the merge commit, the `feat/public-preview-network` Worktrunk worktree and branch were removed, and `moon run root:dev-down` shut down the Kind/Tilt dev stack. Handoff state: public CardanoNetwork preview/preprod/mainnet/custom support is on `master`; public CardanoDBSync supports preview/preprod/custom only as `dedicatedFollower`; public mainnet db-sync, public primarySidecar, public Kupo/faucet, and full live mainnet bootstrap remain explicit future work.
